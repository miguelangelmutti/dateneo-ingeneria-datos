...
# Ahora cada tabla tiene:
#   - incremental: True/False
#   - field: campo de fecha para incremental (None si es full refresh)
DEFAULT_TABLES = {
    "accounts": {"incremental": False, "field": None},
    "accounts_subscription": {"incremental": False, "field": None},
    "subscriptions": {"incremental": False, "field": None},
    "contents": {"incremental": True, "field": "updated_at"},
    "content_attributes": {"incremental": True, "field": "updated_at"},
    "premium_features": {"incremental": True, "field": "updated_at"},
    "account_premium_features": {"incremental": True, "field": "purchase_date"},
    "subscription_payments": {"incremental": True, "field": "payment_date"},
}
...
def carga_datos(
    ...
    for table in tables_to_process:
        ...
        table_conf = DEFAULT_TABLES[table]

        # Determinar si la tabla debe ser full refresh
        is_full_refresh = (
            full_refresh_tables is not None and 
            full_refresh_tables.get(table, False)
        ) or not table_conf["incremental"]  # Si no soporta incremental, es full refresh

        # Configurar carga incremental usando el campo definido solo si la tabla lo soporta
        if not is_full_refresh and table_conf["incremental"] and table_conf["field"]:
            getattr(table_resource, table).apply_hints(
                table_format="iceberg",
                incremental=dlt.sources.incremental(table_conf["field"])
            )
