...
def get_destination_connection(env: str = 'local'):
    ...
    """
    if env == 'local':
        # Usar la base de datos local en el mismo directorio donde se ejecuta el script
        db_path = os.path.join(os.getcwd(), 'datavision.duckdb')
...
def get_dataset_name(env: str) -> str:
    ...
    """
    if env == 'local':
        return 'raw_datavision_local_20250705062220'
    elif env == 'dev':
...
if __name__ == "__main__":
    validacion_conteo = validar_conteo_tablas(
        env='dev',
        tables=['contents', 'content_attributes', 'accounts_subscription']
    )
    mostrar_resumen_conteo(validacion_conteo)

    validacion_duplicados =  validar_duplicados_tablas(
        env='dev',
        tables=['contents', 'content_attributes', 'accounts_subscription']
    )
